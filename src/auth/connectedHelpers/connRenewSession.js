...
// renews session, ONLY SETS auth ON SUCCESS, NO OTHER context such as variables (username, cognitoUser, session ... )
export async function connRenewSessionAsync({ setUnauthStatus, setAuthStatus, username=null, auth, setUsername=null, forceUpdate=false, log="initialValue" }) {
      ...
      if ( (timerDuration < ALLOW_SESSION_RENEW_SECS_BEFORE_EXPIRATION*1000) || forceUpdate) {
        ...
        */
        ...
        const _username = connGetCognitoUsername({ setUnauthStatus, username, log:"for connRenewSession" })
        if (_username === null) return null

        const _cognitoUser = await connNewCognitoUser({ setUnauthStatus, username, log:"for connRenewSession" })
